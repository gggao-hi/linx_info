...
int main()
{
  const char *name = "./file_operator/b.txt";

  const char *buff = "But nearly one third of this has to be spent in sleep; some years have to be spent over our meals; some in making journeys on land and voyages by sea; some in merry-making; some in watching over the sick-beds of our nearest and dearest relatives.\n";
  write_file(open_file(name, O_WRONLY | O_CREAT, S_IRWXU), buff);
  read_file(open_file(name, O_RDONLY, S_IRWXU));
  pwrite_file(open_file(name, O_WRONLY, S_IRWXU), "Your life can be enhanced, and your happiness enriched, when you choose to change your perspective. ");
